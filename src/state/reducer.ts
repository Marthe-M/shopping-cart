...
export function reducer(state: IState, action: Actions): IState {
  switch (action.type) {
    case ActionType.SearchProduct:
      const searchData = state.overview.filter(item => item.description.toLowerCase().includes(action.payload.toLowerCase()))
    ...
    case ActionType.AddProduct:
      ...
      const productIndex = updatedProducts.findIndex(product => product.id === action.payload.id)
      if (productIndex === -1) {
        const newProduct = action.payload
        newProduct.amount = 1
        updatedProducts.push(newProduct)
        return { ...state, products: updatedProducts, lastAddedItem: action.payload, showAddedItem: true };
      }
      else {
        updatedProducts[productIndex] = { ...updatedProducts[productIndex], amount: updatedProducts[productIndex].amount + 1 };
        return { ...state, products: [...updatedProducts], lastAddedItem: action.payload, showAddedItem: true }
      }
    case ActionType.IncreaseItem:
      ...
      copiedProducts[findIndex] = { ...copiedProducts[findIndex], amount: copiedProducts[findIndex].amount + 1 };
    ...
    case ActionType.DecreaseItem:
      ...
      if (copyProducts[findTheIndex].amount > 1) {
        copyProducts[findTheIndex] = { ...copyProducts[findTheIndex], amount: copyProducts[findTheIndex].amount - 1 };
    ...
    case ActionType.RemoveItem:
      ...
      return { ...state, products: [...remainingProducts], lastRemovedItem: action.payload, showRemovedItem: true };
    ...
    case ActionType.ToggleShoppingCart:
      return { ...state, showShoppingCart: !state.showShoppingCart };
    default:
      return state;
  }
}
